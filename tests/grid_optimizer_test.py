import unittest
from src.AutoMLpy import GridHpSearch
...
class TestGridHpOptimizerObjFunc(unittest.TestCase):
    def test_optimize_objective_func(self):
        obj_func_hp_optimizer = ObjectiveFuncHpOptimizer()
        param_gen = GridHpSearch(obj_func_hp_optimizer.hp_space, max_seconds=60, max_itr=10_000)

        save_kwargs = dict(
            save_name=f"obj_func_hp_opt",
            title="Grid search: Objective function",
        )
...
class TestGridHpOptimizerVisionProblem(unittest.TestCase):
    def test_optimize_Cifar10(self):
        ...
        param_gen = GridHpSearch(hp_space, max_seconds=60 * 60 * 1, max_itr=1_000)

        save_kwargs = dict(
            save_name=f"cifar10_hp_opt",
            title="Grid search: Cifar10",
        )
        ...
        self.assertTrue(
            test_acc >= 0.7,
            f"Cifar10 --> Grid search result: {test_acc*100:.3f}%"
        )
        self.assertTrue(
    ...
    def test_optimize_MNIST(self):
        ...
        self.assertTrue(
            test_acc >= 0.985,
            f"MNIST --> Grid search result: {test_acc*100:.3f}%"
        )
        self.assertTrue(
            elapsed_time <= 1.15 * param_gen.max_seconds,
            f"Had a budget of {param_gen.max_seconds}s and take {elapsed_time}s"
